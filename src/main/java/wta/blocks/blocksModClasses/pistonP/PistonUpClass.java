...
public class PistonUpClass extends PistonTreeBlock {
    ...
    private static final PistonKnotClass knot=(PistonKnotClass) BlocksInit.pistonKnot;
    public static int lenghtIn=10;
    public static final HashMap<Direction, VoxelShape> HITBOXES=new HashMap<>();
    public PistonUpClass(Settings settings) {
        super(settings);
        this.setDefaultState((this.stateManager.getDefaultState()).with(ROTATE, Direction.UP));
    }

    private static int distanceOfBack(BlockState state, ServerWorld world, BlockPos pos){
        int ret=0;
        BlockPos posI=pos;
        BlockState StateI;
        do{
    ...
    protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (random.nextInt(2)==0) {
            if (state2.getBlock() == Blocks.AIR) {
                int LenghtInNow = distanceOfBack(state, world, pos);
                ...
                world.setBlockState(pos2, knot.getBlockState(boolMap2.get()), 3);
                world.setBlockState(pos, BlocksInit.pistonIn.getDefaultState().with(ROTATE, state.get(ROTATE)), 3);
                ...
                world.setBlockState(pos2, knot.getBlockState(boolMap2.get()), 3);
                world.setBlockState(pos, BlocksInit.pistonIn.getDefaultState().with(ROTATE, state.get(ROTATE)), 3);
            }else if (state2.isIn(pistonBlocksForSet)){
                ...
                if (!retTrys.isEmpty()){
                    ...
                    world.setBlockState(
                            pos,
                            knot.getBlockState(boolMap.get()),
                            3
                    );
                }else{
